package com.maksimKos.mqttandroid.mqtt
...
class MqttManagerImpl(
    ...
    fun subscribeToTopic(_topics : Array<String>) {
        try {
            mqttAndroidClient?.subscribe(_topics, topicQos, null, object : IMqttActionListener {
                override fun onSuccess(asyncActionToken: IMqttToken) {
    ...
    fun unSubscribeToTopic(_topics : Array<String>) {
        try {
            mqttAndroidClient?.unsubscribe(_topics, topicQos, null)
        } catch (ex: MqttException) {
            ex.printStackTrace()
        }
    }
